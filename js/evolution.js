...
function killLowScoreAnt() {
  ...
  for (var ant of game.ants) if (ant.stats.score < min) my_ant = ant;
...
function reproduceTopScoreAnt() {
  ...
  var my_ant = game.ants[0];
  for (var ant of game.ants) if (ant.stats.score > max) my_ant = ant;
...
export function evolveAnts(strength) {
  ...
  var a = [];
  var b = [];
  for (var i = 0; i < strength; i++) a.push(killLowScoreAnt());
  for (var i = 0; i < strength; i++) b.push(reproduceTopScoreAnt());
  for (var ant of game.ants) ant.stats.score = 0;
